...
class MedViTExtractor(nn.Module):
    def __init__(self, vision_model):
        super(MedViTExtractor, self).__init__()

        if 'small' in vision_model.lower():
            self.model = MedViT_small()
        elif 'base' in vision_model.lower():
            self.model = MedViT_base()
        else:
            self.model = MedViT_large()
